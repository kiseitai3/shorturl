...
def shorturl_server():
    ...
    shorturl_server_socket.bind((shorturl_HOST, shorturl_PORT))
    while True:
        shorturl_server_socket.listen(1)
        client,addr = shorturl_server_socket.accept()
        if client is not None:
            ...
            #use shorturl
            if make_new_url:
                new_url = shorturl_new(data)
            else:
                new_url = shorturl_getRealURL(shorturl_parse_alias(data))
            #send to client new url and close connection
            client.send(new_url)
            #sanitize for the next connection
            new_url = ''
            #Disconnect client
            client.close()
        #Reset state/ more sanitizing
        server_chat = True
    #Clean shorturl
    shorturl_close()
